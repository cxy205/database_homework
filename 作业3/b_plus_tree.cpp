...
bool BPLUSTREE_TYPE::GetValue(const KeyType &key,
  ...
  B_PLUS_TREE_LEAF_PAGE_TYPE *tar = FindLeafPage(key,false,OpType::SEARCH,transaction);
  result.resize(1);
  
  if (tar == nullptr)
    return false;
  else{
    bool res = tar->Lookup(key,result[0],comparator_);
...
void BPLUSTREE_TYPE::StartNewTree(const KeyType &key, const ValueType &value) {
  ...
  assert(root!=nullptr);
  B_PLUS_TREE_LEAF_PAGE_TYPE *rp = reinterpret_cast<B_PLUS_TREE_LEAF_PAGE_TYPE *>(root->GetData());
  
  root_page_id_=newid;
  rp->Init(newid,INVALID_PAGE_ID);
  rp->Insert(key,value,comparator_);
...
bool BPLUSTREE_TYPE::InsertIntoLeaf(const KeyType &key, const ValueType &value,
  ...
  ValueType v;
  bool exist = lp->Lookup(key,v,comparator_);
  if (exist) {
    FreePagesInTransaction(true,transaction);
    return false;
  }
  lp->Insert(key,value,comparator_);
  if (lp->GetSize() > lp->GetMaxSize()) {//insert then split
    B_PLUS_TREE_LEAF_PAGE_TYPE *np = Split(lp,transaction);//unpin it in below func
    InsertIntoParent(lp,np->KeyAt(0),np,transaction);
  }
...
template <typename N> N *BPLUSTREE_TYPE::Split(N *node, Transaction *transaction) { 
  page_id_t newid;
  Page* const np = buffer_pool_manager_->NewPage(newid);
  assert(np != nullptr);
  np->WLatch();
  transaction->AddIntoPageSet(np);
  
  N *newNode = reinterpret_cast<N *>(np->GetData());
...
void BPLUSTREE_TYPE::InsertIntoParent(BPlusTreePage *old_node,
  ...
  assert(page != nullptr);
  BPlusTree *parent = reinterpret_cast<B_PLUS_TREE_INTERNAL_PAGE_TYPE *>(page);
  new_node->SetParentPageId(parentId);
  parent->InsertNodeAfter(old_node->GetPageId(), key, new_node->GetPageId());
  if (parent->GetSize() > parent->GetMaxSize()) {
    B_PLUS_TREE_INTERNAL_PAGE_TYPE *newLeafPage = Split(parent,transaction);//new page need unpin
...
bool BPLUSTREE_TYPE::FindLeftSibling(N *node, N * &sibling, Transaction *transaction) {
  ...
  B_PLUS_TREE_INTERNAL_PAGE_TYPE *parent = reinterpret_cast<B_PLUS_TREE_INTERNAL_PAGE_TYPE *>(page);
...
bool BPLUSTREE_TYPE::AdjustRoot(BPlusTreePage *old_root_node) {
  ...
  if (old_root_node->GetSize() == 1) {// case 1
    ...
    assert(page != nullptr);
    B_PLUS_TREE_INTERNAL_PAGE_TYPE *newRoot =
...
INDEXITERATOR_TYPE BPLUSTREE_TYPE::Begin() { 
  KeyType useless;
  B_PLUS_TREE_LEAF_PAGE_TYPE *start_leaf = FindLeafPage(useless, true);
...
int BPLUSTREE_TYPE::isBalanced(page_id_t pid) {
  ...
  int ret = 0;
  if (!node->IsLeafPage())  {
    auto page = reinterpret_cast<B_PLUS_TREE_INTERNAL_PAGE_TYPE *>(node);
    int last = -2;
